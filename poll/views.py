...
"""def poll_view_2(request, question_id):
    ...
    try:
        ...
        form = VoteForm_2(instance=previous_vote, question_id=question_id)
    except Vote_2.DoesNotExist:
        form = VoteForm_2(question_id=question_id)

    if request.method == 'POST':
        form = VoteForm_2(request.POST, question_id=question_id)
        if form.is_valid():
            vote = form.save(commit=False)
            vote.question = question
            vote.user = user
